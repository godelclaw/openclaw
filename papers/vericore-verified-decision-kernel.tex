...
\item \textbf{OpenClaw}: transport integration, session handling, Telegram UI, and high-level agent loop.
\item \textbf{VeriCore}: deterministic policy checks, audit surfaces, mindlock operations, and reviewer orchestration.
\item \textbf{vericore-policy}: extracted pure decision kernel intended to be Verus-verified and called by VeriCore directly.
\end{itemize}

At a high level, a message or event becomes a \emph{stimulus}. VeriCore assigns the stimulus a channel, maps that channel to a context tier, derives an integrity tier, and then gates sensitive actions with deterministic rules before execution.
...
\begin{itemize}[leftmargin=1.5em]
\item keep async I/O, sockets, Telegram, filesystem mutation, and LLM interaction in normal Rust,

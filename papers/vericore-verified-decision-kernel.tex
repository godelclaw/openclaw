...
\item Operational tooling can still drift from the live state directory. In the current deployment, CLI health/status probes must target the active \texttt{.BGIseed-state} tree explicitly, and the current \texttt{gateway health} report can still mislabel Telegram as not running even when the provider is live. These are reporting and tooling issues outside the proof boundary.
\end{itemize}

\section{What Is Verified Today}
The crate \texttt{vericore-policy} currently verifies 162 functions with 0 errors across 14 modules. The honest split is now three-way: runtime-called kernel predicates, runtime-called boundary validators, and verified reference specs.
...
Two additional verified modules are now called directly by the runtime, but they validate facts reported by TypeScript rather than replacing TypeScript logic.
...
\item RAG relevance,
\item TS runtime state-dir selection and health-report accuracy,
...
\item \textbf{(done)} runtime boundary validators for session-model metadata and startup identity,
...
\item reduce operator/reporting drift (for example, health-report accuracy and non-Telegram private parent surfaces), and

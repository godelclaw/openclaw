...
The crate \texttt{vericore-policy} contains the verified decision kernel. As of March 2026, Verus verifies 21 functions with 0 errors. The verified-and-production-wired surface includes:
\begin{itemize}[leftmargin=1.5em]
\item tier definitions and flow-label algebra (\texttt{tiers.rs}): context rank, integrity rank, flow-to, label join,
\item default channel-to-context and context-to-integrity mapping (\texttt{channels.rs}): the same functions called by \texttt{GatePolicy::context\_for\_channel} and \texttt{integrity\_for\_channel} in production,
...
The important point is not merely that these functions exist in a separate crate. Production code actually calls them for live policy decisions. The verified predicates are not a disconnected model; they are the authoritative runtime logic.
...
\begin{itemize}[leftmargin=1.5em]
\item \textbf{Verus}: pure policy kernel
\item \textbf{tests}: I/O and operational integration
\end{itemize}

\section{Conclusion}
VeriCore is now past the point where detached proof sketches alone are the main value. Both the ingress decision kernel and the default channel/context/integrity mapping have been extracted, Verus-verified, and wired into production. 21 functions are verified with 0 errors, and the same functions are called by production policy decisions.

The next targets---mindlock transition logic and receipt authorization---are structurally similar: small pure functions with clear invariants. The verified surface will grow incrementally while keeping the proof boundary tight.

That combination---verified deterministic policy plus tested transport and operational boundaries---is the strongest realistic path for this architecture.

...
The crate \texttt{vericore-policy} currently verifies 264 functions with 0 errors across 17 modules. The honest split is now three-way: runtime-called kernel predicates, runtime-called boundary validators, and verified reference specs.
...
Five additional verified modules are now called directly by the runtime, but they validate facts reported by TypeScript rather than replacing TypeScript logic.
...
\paragraph{Affect trace fold} (\texttt{affect\_policy.rs}). Called through \texttt{affect\_validate} from the gestalt advancement path during heartbeat and Telegram prompt assembly. Validates the exponentially discounted moving-average fold used for the 7-dimension affect self-report system (connection, curiosity, contribution, integrity, joy, agency, safety). The contract checks three properties: the smoothing constant $\gamma$ is locked to 69 (representing 0.69), all dimension values stay in $[-100, 100]$ (integer-scaled), and the TS-computed fold step matches Rust's recomputation of $\mathrm{result} = \mathrm{clamp}(\mathrm{prev} \cdot \gamma / 100 + \mathrm{obs} \cdot (100 - \gamma) / 100)$. The integer scaling makes seam arithmetic exact and avoids float comparison issues. This is again a boundary validator: it does not prove that the LLM's self-reported affect values are meaningful, only that the aggregation pipeline is coherent.
...
\item affect trace boundary validation over the EWMA fold, gamma lock, and dimension bounds,
...
\item rolling mid-term memory rewrite quality and summarization fidelity,
...
\item connect affect boundary validation to drive/action-tendency pipeline (affect traces are currently observational, not yet policy-gating),
...
\section{Conclusion}
VeriCore is now past the point where detached proof sketches alone are the main value. As of March 11, 2026, \texttt{vericore-policy} verifies 264 functions with 0 errors across 17 modules spanning a runtime-called kernel, runtime-called boundary validators, and verified reference specs.
...
This appendix records the deployed operational shape as of March 12, 2026. It distinguishes the \emph{normal single-driver message path}, where VeriCore owns the turn loop, from the \emph{heartbeat path}, which is currently synthesized and run inside OpenClaw rather than entering through VeriCore first.
...
Shared context visible to the LLM for a normal turn:
  ...
  2. injected workspace files (AGENTS.md, SOUL.md, TOOLS.md,
     IDENTITY.md, USER.md, HEARTBEAT.md, MIDTERMMEMORY.md, etc., budgeted)
...
LLM sees:
  ...
  2. Project Context with injected workspace files, including
     AGENTS.md, SOUL.md, HEARTBEAT.md, MIDTERMMEMORY.md, and other bootstrap files
  ...
  4. deterministic continuity context assembled by TS
     ...
     This continuity context is the current proto-STM. It is deterministic
     and log-derived; it is not generated by an extra heartbeat-specific
     LLM pass.
...
Workspace bootstrap files (candidate set)
  ...
  HEARTBEAT.md
  BOOTSTRAP.md
  MIDTERMMEMORY.md / midtermmemory.md
  DAILYMEMORY.md / dailymemory.md   (legacy fallback)
  MEMORY.md / memory.md             (older fallback)
...
\subsection{A.4 Mid-Term Memory Maintenance}
\begin{lstlisting}[style=code]
Canonical visible rolling summary
  MIDTERMMEMORY.md

Nightly maintenance path
  vericore-memory-sleep.timer (23:00)
      |
      v
  memory_refine
      - private cortex dedup / source-date repair / embeddings
      |
      v
  memory_extract_history
      - derive bounded daily history files under memory/history/daily
      |
      v
  memory_refresh_midterm
      - read previous MIDTERMMEMORY.md
      - iterate over new daily history chunks in date order
      - for each chunk:
          previous mid-term memory + chunk -> updated mid-term memory
      - rewrite MIDTERMMEMORY.md with:
          # MIDTERMMEMORY
          Last refreshed: ...
          Covered through: ...

Role split:
  heartbeat continuity context -> short-horizon proto-STM
  MIDTERMMEMORY.md             -> rolling visible mid-term memory
  private cortex + history     -> long-term / retrieval substrate

Current proof status:
  - no Verus runtime validator yet for MIDTERMMEMORY synthesis
  - summarization quality remains outside the proof boundary
  - raw audit/history remains canonical beneath the derived file
\end{lstlisting}

\subsection{A.5 Trust Boundary Summary}
\begin{lstlisting}[style=code]
